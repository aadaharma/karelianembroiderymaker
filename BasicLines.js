...
export function checkForCollision(templateId){
    ...
    const oldIndex = pattern.findIndex(item => item.x === gridX && item.y === gridY && item.templateId === templateId);
            if (oldIndex === -1){
                if (templateId === 'diag-right' && pattern.some(item => item.x === gridX && item.y === gridY && item.templateId === 'cross')){
                    return true;
                }
                if (templateId === 'diag-left' && pattern.some(item => item.x === gridX && item.y === gridY && item.templateId === 'cross')){
                    return true;
                }
                if (templateId === 'line-left' && pattern.some(item => item.x === gridX && item.y === gridY && item.templateId === 'square')){
                    return true;
                }
                if (templateId === 'line-right' && pattern.some(item => item.x === gridX && item.y === gridY && item.templateId === 'square')){
                    return true;
                }
                if (templateId === 'line-top' && pattern.some(item => item.x === gridX && item.y === gridY && item.templateId === 'square')){
                    return true;
                }
                if (templateId === 'line-bottom' && pattern.some(item => item.x === gridX && item.y === gridY && item.templateId === 'square')){
                    return true;
                }
